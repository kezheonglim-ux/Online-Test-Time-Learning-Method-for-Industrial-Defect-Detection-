...
# Calibrate a consistency threshold from trusted good_* prediction logs.
import pandas as pd
import numpy as np
import json
...
# Read the prediction log containing consistency measurements.
df = pd.read_csv(LOG_FILE)

# Use trusted good_* samples only.
df = df[df["file_name"].str.startswith("good_", na=False)]

# Calibrate each category independently.
for category, group in df.groupby("category"):
    ...
    # Use the 95th percentile of trusted-normal consistency errors.
